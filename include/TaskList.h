...
namespace T_Threads {
	...
	struct LMarkablePointer {
		...
		LNodeBase* getReference() const {
			return unpackPtr(ref_.load(std::memory_order_acquire));
		}

		bool getMark() const {
			return unpackMark(ref_.load(std::memory_order_acquire));
		}
	...
	class TaskList {
		...
		LNodeBase* tail;
	public:
		TaskList(Arena* arena) : arena_(arena) {
			void* memHead = arena_->allocate(sizeof(LNode<T>));
			void* memTail = arena_->allocate(sizeof(LNode<T>));
		...
		bool add(uint64_t key, T item) {
			while (true) {
				...
				void* mem = arena_->allocate(sizeof(LNode<T>));
